...
namespace ELGame
{
          ...
          IVisualRenderer<BattleField, BattleFieldRenderer>
    {
        ...
        private void OnBattleUnitAndGridTouched(GridUnit gridTouched, BattleUnit battleUnitTouched)
        {
            ...
            if (manualOperationState == ManualOperationState.None)
            {
                ...
                if (battleUnitTouched != null)
                {
                    ...
                    {
                        Debug.Log("非操作状态点中了非手动操作的战斗单位:" + battleUnitTouched.ToString());
                    }
                }
                //点中了地图
                else
                {
                    Debug.Log("非操作状态点中了地图单位:" + gridTouched.ToString());
                }
            }
            //选择移动状态
            else if (manualOperationState == ManualOperationState.Move)
            {
                ...
                if (battleUnitTouched != null)
                {
                    ...
                    {
                        Debug.Log("移动状态点中了非手动操作的战斗单位:" + battleUnitTouched.ToString());
                    }
                }
                //点中了地图
                else
                {
                    ...
                    {
                        Debug.Log("移动状态点中了障碍格子:" + gridTouched.ToString());
                    }
                    else
                    {
                        //点击是否超出了范围
        ...
        bool activeManualList = false;
        
        private void OnGUI()
        {
            if (!activeManualList || manualBattleUnitRenderer == null)
                return;

            //可以移动，必可以攻击
            if (manualBattleUnitRenderer.battleUnit.CheckManualState(ManualActionState.Move))
            {
                Vector3 pos = BattleCamera.WorldToScreenPoint(manualBattleUnitRenderer.transform.position);
                pos.y = Screen.height - pos.y;
                int selected = GUI.SelectionGrid(new Rect(pos.x, pos.y, 100, 100), -1, new string[3] { "移动", "攻击", "待命" }, 1);
                if (selected == 0)
                {
                    BeforeManualMove();
                }
            ...
            //只可以攻击
            else if (manualBattleUnitRenderer.battleUnit.CheckManualState(ManualActionState.Skill))
            {
                Vector3 pos = BattleCamera.WorldToScreenPoint(manualBattleUnitRenderer.transform.position);
                pos.y = Screen.height - pos.y;
                int selected = GUI.SelectionGrid(new Rect(pos.x, pos.y, 100, 50), -1, new string[2] { "攻击", "待命" }, 1);
                if (selected == 0)
                {
                    BeforeManualSkill();
                }
