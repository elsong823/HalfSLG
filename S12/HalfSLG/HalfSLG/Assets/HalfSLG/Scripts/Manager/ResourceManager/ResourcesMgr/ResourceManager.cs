...
namespace ELGame.Resource
{
        ...
        :Editor
    {
        public override void OnInspectorGUI()
        {
            ...
            {
                if (GUILayout.Button("查看状态"))
                {
                    ResourceManager resManager = target as ResourceManager;
                    if (resManager != null)
                        resManager.Desc();
                }
            }
        ...
        : BaseManager<ResourceManager>
    {
...
#if UNITY_EDITOR
        public bool editorMode = true;    //在编辑器上直接读取
#else
        public bool editorMode = false;   //在编辑器上直接读取
#endif

        [Space()]
        public bool unloadAllLoadedObjects = true;  //在Unload时是否彻底卸载资源

        [Header("Async setting:"), Space()]
        ...
        [Range(0, 10)] public int recycleDepth = 0;
        [Range(-1f, 1f)] public float recycleUpdateInterval = 0.01f;
        [Range(1, 10)] public int recycleCountLimit = 2;   //每次回收的数量
