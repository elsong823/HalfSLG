...
namespace ELGame
{
    ...
    public class GameEventHandlerItem
    {
        class EventInfo
        {
            ...
            public EventInfo(object listener, GameEventHandler handler, int time)
            {
        ...
        //new就等于新增
        public GameEventHandlerItem(string key, object _ref, GameEventHandler _handler, int _time)
        {
            eventKey = key;
        ...
        public void Remove(object listener)
        {
            for (int i = eventInfos.Count - 1; i >= 0; --i)
            {
                if (eventInfos[i].listener.Equals(listener))
                {
                    //移除
                    gameEvent -= eventInfos[i].handler;
                    eventInfos.RemoveAt(i);
        ...
        public override string ToString()
        {
            ...
            for (int i = 0; i < eventInfos.Count; i++)
            {
                string _name = (eventInfos[i].listener is GameObject) ? ((GameObject)eventInfos[i].listener).name : eventInfos[i].listener.GetType().ToString();
                strBuilder.AppendFormat("{0}, Name:{1}, Time:{2}", i, _name, eventInfos[i].time);
            }
        ...
        : BaseManager<EventManager>
    {
            ...
            Dictionary<string, GameEventHandlerItem> dic = null)
        {
            ...
            {
                eventListItem.Value.Remove(reference);
