...
#if UNITY_EDITOR
    [CustomEditor(typeof(BattleUnitRenderer))]
    public class BattleUnitRendererCustomEditor
        : Editor
    {
        public override void OnInspectorGUI()
        {
            ...
            if (GUILayout.Button("打印仇恨列表"))
            {
                BattleUnitRenderer battleUnitRenderer = target as BattleUnitRenderer;
                if (battleUnitRenderer != null)
                {
                    Debug.LogFormat("打印仇恨列表( UID = {0} )...", battleUnitRenderer.battleUnit.battleUnitID);
                    Debug.Log(battleUnitRenderer.battleUnit.hatredRecorder.Desc());
                }
            }
            else if (BattleFieldRenderer.Instance.editor_ManualOptBattleUnit != null)
            {
                BattleUnitRenderer battleUnitRenderer = target as BattleUnitRenderer;
                if (battleUnitRenderer && GUILayout.Button("查看技能效果分析"))
                {
                    BattleCalculator.Instance.AutoReleaseAnalysisor.Ananlysis(
                        BattleFieldRenderer.Instance.editor_ManualOptBattleUnit,
                        battleUnitRenderer.battleUnit,
                        BattleFieldRenderer.Instance.editor_ManualOptBattleUnit.battleUnitAttribute.battleSkills);

                    Debug.Log(BattleCalculator.Instance.AutoReleaseAnalysisor.Desc());
